...
//draw
        ...
        float cameraPosX = getClampedValue(getCameraX(*m_mario), 0.0f, m_mostRightX);        QGraphicsRectItem* bgRect = new QGraphicsRectItem();
...
float GameScene::getClampedValue(float value, float low, float high) {
    if (value < low) {
        return low;
    } else if (value > high) {
        return high;
